...
@WebServlet("/EditTrains")
public class EditTrain2 extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		LogicClass pl =  new LogicClass();
				...
				try {
					...
					int res = pl.updateTrain(f);
					if(res != 0)
					{
						  req.setAttribute("trains",pl.findAll());
				    	  req.setAttribute("message1","Train updated successfully");
				    	  RequestDispatcher rd = req.getRequestDispatcher("Home.jsp") ;
				    	  rd.include(req, resp);
					}
					else
					{
						 req.setAttribute("massage1","Train update failed");
				    	  RequestDispatcher rd = req.getRequestDispatcher("Home.jsp") ;
				    	  rd.include(req, resp);
					}
